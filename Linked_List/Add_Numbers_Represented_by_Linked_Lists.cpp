...
{ 
	...
	if(head1->next==NULL)
	{
	    Node *res = new Node(((head1->data)+(head2->data))%10);
	    res->next=NULL;
	...
	{
	    Node *res = addSameSize(head1->next,head2->next,carry);
	    Node *newNode = new Node(((head1->data) + (head2->data) + (*carry) )%10);
	    newNode->next=res;
...
{ 
	...
	Node *newNode = new Node(((*carry)+(head1->data))%10);
